...
fn default_pre_compact(code: &str) -> String {
    // Must NOT start with a `/`. The orchestrator sends this file's content
    // as a regular prompt to claude's TUI; any leading `/`-prefixed line is
    // interpreted as a slash command and rejected with "Args from unknown
    // skill". Plain header text first, slash command (real built-in /compact)
    // only at the end where it actually exists.
    format!("Pre-compact summary for {}.\n\nSummarise (200–500 words):\n- Tickets touched this session (human ids + verdict).\n- Open questions you didn't get to.\n- Files modified.\n- Lessons learnt (file them via the pending-lessons sentinel BEFORE /compact - see your kickoff).\n- State you'll need to resume cleanly post-compact.\n\nThen trigger /compact.\n", code)
}
